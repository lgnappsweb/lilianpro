          ...
          {filteredProducts.map((product) => (
            <Card key={product.id} className="bg-background border-4 border-primary/5 rounded-2xl p-4 sm:p-6 shadow-xl hover:border-primary/20 transition-all flex flex-col justify-between w-full relative overflow-hidden group">
              ...
              <div className="flex flex-row items-center justify-center gap-2 w-full pt-3 sm:pt-4 border-t-2">
                <Button variant="outline" asChild className="h-10 sm:h-14 font-black text-[11px] sm:text-[15px] uppercase tracking-tighter rounded-lg sm:rounded-xl border-2 hover:bg-primary/5 px-2 flex-1 shadow-sm">
                  <Link href={`/produtos/${product.id}`}>
                    <FileText className="mr-1 size-4 sm:size-5" />
                    Detalhes
                  </Link>
                </Button>
                <Button variant="outline" asChild className="h-10 sm:h-14 font-black text-[11px] sm:text-[15px] uppercase tracking-tighter rounded-lg sm:rounded-xl border-2 hover:bg-primary/5 px-2 flex-1 shadow-sm">
                  <Link href={`/produtos/${product.id}/editar`}>
                    <Edit className="mr-1 size-4 sm:size-5" />
                    Editar
                  </Link>
                </Button>
                <Button 
                  variant="outline" 
                  className="h-10 sm:h-14 font-black text-[11px] sm:text-[15px] uppercase tracking-tighter rounded-lg sm:rounded-xl border-2 text-destructive border-destructive/20 hover:bg-destructive/5 hover:border-destructive px-2 flex-1 shadow-sm"
                  onClick={() => setProductToDelete(product)}
                >
                  <Trash2 className="mr-1 size-4 sm:size-5" />
                  Excluir
                </Button>
              </div>
            </Card>
